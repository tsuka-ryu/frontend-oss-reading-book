...
- 1回に1章。全パートを `spec/BOOK_SPEC.md` 第0節の表の順に1つずつ回す（第0.1節のサイクル）。次に書くのは、下の「サイクル」で「次」と書かれたパートである。そのパートの章リストで、`[~]` があればその続き、なければ上から最初の `[ ]` を書く。
- 書いたPRの中で、「サイクル」の「次」を一つ先のパートへ移す（Servoの次はシグナル。サイクルから外れたパートは飛ばす）。
- 一つのパートは約30章。章立て（`draft/<slug>/plan-30.md`）がないパートの番が来たら、その回はパス1で章立てを作り、その章の一覧をこのファイルのパートの節に写すところまでとする。
- 状態は `[ ]` 未着手 / `[~]` 着手中 / `[x]` 完了 の3つ。章ごとに付ける。パス2〜5は章ごとに回し、パスごとに別のコミットにする。
- 約30章を書き終えたパートは、「サイクル」から外す。
