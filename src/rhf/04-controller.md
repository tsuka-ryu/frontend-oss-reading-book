...
`"addresses"`を購読した者にとって、`"addresses.0.city"`の変更は自分に関係のある話のはずである。
ところが文字列として比べる限り、二つはただの別の名前でしかない。
名前の中のドットは、実は木の枝分かれを表している。
名前を文字列ではなく木の道筋として読むと、誰に知らせるべきかはどう変わるのか。
（番号の`0`が削除や並べ替えで振り直される問題は、もっと先で`useFieldArray`と一緒に扱う。）
