...
依存配列を人間に書かせないという約束の裏に、これだけのものが並んでいた。

次のパートのReact Hook Formは、Reactの上で動く。
Reactでは、関数を呼び直す単位はコンポーネントで、呼び直すことを**再描画**という。
そこにも、[formStateのどのキーを読んだかを覚える](../rhf/02-proxy-form-state.md)仕組みが出てくる。
読んだものを記録するという同じ発想が、Reactの中ではどんな形になるのか。
