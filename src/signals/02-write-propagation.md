...
setterを続けて三回呼んだら、画面は三回描き直されるのか。
...
このループの外側では、`Updates` の長さが100万を超えたときに「Potential Infinite Loop Detected」を投げる保険も掛かっている。

積まれたキューを流すのが、抜粋にも登場した `runUpdates` と、その終端で呼ばれる `completeUpdates` の組である。
`runUpdates` は入れ子を検知する。
すでに消化の最中なら渡された関数を実行して積むだけにし、いちばん外側の呼び出しだけがキューを流す。
だから、素のsetterを三回続けて呼べば消化も三回起きるが、三回の書き込みを `batch` で囲めば、消化は囲みを抜けるときの一回にまとまる。
その `batch` の実装は `runUpdates(fn, false)` を呼ぶだけの一行で、特別な仕掛けは書き込み経路と共有している。
