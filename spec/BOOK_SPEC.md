...
この順は、読者が通して読むときの順（目次の順）である。
後のパートは前のパートを前提にしてよい。ただし執筆は全パートを並行して進める（第0.1節）ので、前提にする章がまだ書かれていないことがある。そのときは前のパートの内容を一段落で補い、リンクは章ができてから張る。
...
執筆は1回に1章で、**上の表のパートを表の順に1つずつ回す**（サイクル）。
Servoの次はシグナルに戻る。
同じOSSを続けて書かない。
次に書くパートは `BACKLOG.md` の「サイクル」に書いてあり、1回書くごとに一つ先へ進める。

章立て（`draft/<slug>/plan-30.md`）がまだないパートに順番が回ってきたら、その回はそのパートのパス1（約30章の章立て）だけを行う。
したがって最初の1周では、多くのパートが章立てで終わる。
約30章を書き終えたパートは、サイクルから外す。

パートを回すのは、読者が毎回違う題材を読めるようにするためと、一つのOSSの語彙に書き手が慣れすぎて、説明を飛ばすようになるのを防ぐためである。
前者の代わりに、同じパートの次の章までの間が空く。
書く前に、同じパートの直前の章を読み返してから書く。
...
本文（コードブロックと表を除く）は1,500字から4,000字に収める。
