...
- 読者は、Web仕様・ブラウザ実装・フロントエンドフレームワーク・コンパイラの内部を理解したいエンジニア
...
- 最初は対象を広く取り、件数も絞らない。ただし詳しく書くのは興味深いものだけにし、それ以外は一言にまとめる。読者が号を眺めて、いらない対象をこのファイルから外していく
- ランタイム（Node.js、Bun）やバンドラ（Rolldown）の内部実装は、読者の興味の外なので対象にしない
...
拾ったニュースを、除外の判定に当たるもの以外はすべて載せる。ただし全件を詳しく書くと読み切れないので、「注目」と「その他」に分けて書き分ける。トップニュースや深掘りで扱ったものは、見出しと参照リンクだけ。

- 対象ごと（リポジトリや仕様ごと）に `### <対象名>` でまとめる。読者が対象ごとに要不要を判断できるようにするため
- 注目：内部設計や仕様の考え方を学べる、興味深いものだけ。`#### <見出し>` を立て、要約を2〜3行と「なぜ変えたか」を1行書く
  - 当たるもの：新しい機能・APIや設定、利用者に見える挙動の変更、設計や仕組みが変わる変更（アルゴリズム、データ構造、アーキテクチャ）、背景の議論に学びがあるもの
  - 1対象あたり0〜3件を目安にする。該当がなければ注目は置かない
- その他：注目に当たらないもの（狭い条件のバグ修正、リファクタリング、内部の整理、依存更新、ツールやビルドまわりなど）は、注目の後に箇条書きで1件1行にまとめる
  - 書式：`- <何が変わったかを一言>（<PRへのリンク>）`。分野タグと「なぜ変えたか」は省く
  - 見出しは立てない
- 対象の中では、注目もその他も影響の大きい順に並べる
...
#### PPFでシェルの静的化を設定で選べるようになる

【Next.js】（要約2〜3行）
なぜ変えたか：（1行）
出典: https://github.com/vercel/next.js/pull/98191

その他：

- Cached Navigationsでコンテンツが欠落する不具合を修正（[#98767](https://github.com/vercel/next.js/pull/98767)）
...
ルール:
- 除外の判定に当たらないものは、件数を絞らずすべて載せる。詳しく書くか一言にするかは「号の形式」の「注目」と「その他」に従う
