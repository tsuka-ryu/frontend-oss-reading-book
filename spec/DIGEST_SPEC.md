...
Web仕様と、フロントエンドのツールチェーン（当面はoxc）の動きを、毎朝届くニュースとしてまとめ、`src/digest/` に積む。
...
- 本文の先頭に分野タグを付ける：`【HTML】` `【DOM】` `【Fetch】` `【Streams】` `【URL】` `【TC39】` `【CSS】` `【ARIA】` `【WCAG】` `【ServiceWorker】` `【oxc】` `【React】` `【Next.js】` `【脆弱性】`
...
その日のニュースから、内部設計を学ぶ題材として最も価値のある変更を1つ選び、ソースを読んで解説する。トップニュースと同じでもよい。
読了5分程度を目安にする。
...
- 各ニュースは要約を2〜3行
- 影響の大きい順に並べる。分野を分けずに混ぜる
- トップニュースと合わせて10件まで。超えるときは影響の小さいものから落とす
...
- 実行の最後の返答を、通知の本文になる形にする
  - 1行目：今日のひとこと
  - 続けて：トップニュース、今日の深掘り、ヘッドラインの見出しを1行ずつ（最大5件）
